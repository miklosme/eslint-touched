...
function printLinterOutput(res, config, webpack) {
    ...
    if (
        ...
        if (res.errorCount || res.warningCount) {
            ...
            // default behavior: only emimitting warnings
            var emitter = webpack.emitWarning

            // force emitError if user want this
            if (config.emitError) {
...
module.exports = function(input, map) {
    ...
    listTouchedLines(webpack.resourcePath)
        .then(touchedLines => {
            const thisFile = touchedLines.find(x => webpack.resourcePath.endsWith(x.file));
            if (thisFile && thisFile.lines.length) {
                const report = lint(engine, input, resourcePath);
                const filteredResults = filterNonTouched(report.results, touchedLines);
                const filteredReport = {
                    results: filteredResults,
                    errorCount: filteredResults.reduce((sum, { errorCount }) => sum + errorCount, 0),
                    warningCount: filteredResults.reduce((sum, { warningCount }) => sum + warningCount, 0)
                }
                printLinterOutput(filteredReport, config, webpack);
            }
